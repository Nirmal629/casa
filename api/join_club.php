...
try {
    ...
    if (!$club_query) {
        throw new Exception('Database schema error (ca_clubs): ' . $conn->error);
    }
    ...
    if (!$check_query) {
        throw new Exception('Database schema error (ca_player_club_status): ' . $conn->error);
    }
    ...
    if (!$player_query) {
        throw new Exception('Database error (ca_users): ' . $conn->error);
    }
    ...
    if (!$insert_query) {
        throw new Exception('Database schema error (insert_ca_player_club_status): ' . $conn->error);
    }
    ...
    if ($insert_query->execute()) {
        ...
        throw new Exception('Database execute error: ' . $conn->error);
    }
} catch (Exception $e) {
    echo json_encode(['status' => 'error', 'message' => 'PHP/DB Crash: ' . $e->getMessage()]);
}
