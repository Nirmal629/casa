...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Fetch all games played by the user in the selected month/year
    $query = "
        SELECT 
    ...
    if ($count > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // ✅ FIX: Do NOT filter payments by payment date, just match by GAME_ID and USER_ID
            ...
            if ($due == 0 && $payData['STATUS'] === 'N') {
                $actionHtml = "
                    <button class='btn btn-success btn-sm approveBtnnn' data-id='{$game_id}' data-user='{$user_id}'>Approve</button>
                    <button class='btn btn-danger btn-sm rejectBtnnn' data-id='{$game_id}' data-user='{$user_id}'>Reject</button>
                ";
            } elseif ($paid == 0) {
                $actionHtml = "<button class='btn btn-warning btn-sm payBtnnn' data-id='{$game_id}' data-user='{$user_id}' data-due='{$due}'>Pay</button>";
            } else {
                $actionHtml = "
